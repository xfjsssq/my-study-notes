...
export function parseVideoUrl(url: string, title?: string): Video | null {
  const bv = url.match(BV_REGEX);
  if (bv) return { platform: "bilibili", id: bv[0], title: title || "B站视频", url };
  const yt = url.match(YT_REGEX);
  if (yt) return { platform: "youtube", id: yt[1], title: title || "YouTube 视频", url };
...
export interface VideoEntry { title: string; url: string; platform?: VideoPlatform }
