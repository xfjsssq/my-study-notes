...
export function getChapterProgress(
  ...
  return bundle.subjects[subjectId]?.chapters?.[`${subjectId}/${chapterId}`] ?? null;
}
...
export function markChapterDone(
  ...
  if (!bundle.subjects[subjectId]) {
    bundle.subjects[subjectId] = { subjectId, updatedAt: "", chapters: {} };
  }
...
export function getProgressStats(subjectId: string): {
  ...
  const done = Object.values(subject.chapters).filter((c) => c.status === "done").length;
...
export async function syncProgressToGitHub(bundle: ProgressBundle): Promise<{ sha: string } | null> {
  ...
  try {
    ...
    if (existing.ok) { const info = await existing.json(); sha = info.sha; }
    const content = btoa(unescape(encodeURIComponent(JSON.stringify(bundle, null, 2))));
    const body: Record<string, string> = { message: `Sync progress: ${bundle.updatedAt}`, content };
    if (sha) body.sha = sha;
    ...
    if (!resp.ok) { console.error("Sync failed:", await resp.json()); return null; }
