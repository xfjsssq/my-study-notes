...
async function deriveKey(pat: string): Promise<CryptoKey> {
  ...
  const keyMaterial = await crypto.subtle.importKey("raw", enc.encode(pat), KEY_ALGO, false, ["deriveKey"]);
  return crypto.subtle.deriveKey(
    { name: KEY_ALGO, salt: getSalt(), iterations: ITERATIONS, hash: "SHA-256" },
    keyMaterial,
